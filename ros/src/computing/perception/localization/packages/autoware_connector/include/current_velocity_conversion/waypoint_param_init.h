...
namespace waypoint_maker
{
    ...
    void waypoint_param_init(autoware_msgs::Waypoint *wp, const std::unordered_map<std::string, std::string> map, const unsigned int id_counter)
    {
        wp->pose.pose.position.x = (map.find("x") != map.end()) ? std::stod(map["x"]) : 0;
        wp->pose.pose.position.y = (map.find("y") != map.end()) ? std::stod(map["y"]) : 0;
        wp->pose.pose.position.z = (map.find("z") != map.end()) ? std::stod(map["z"]) : 0;
        ...
        wp->twist.twist.linear.x = (map.find("velocity") != map.end()) ? kmph2mps(std::stod(map["velocity"])) : 0;
        wp->change_flag = (map.find("change_flag") != map.end()) ? std::stoi(map["change_flag"]) : 0;
