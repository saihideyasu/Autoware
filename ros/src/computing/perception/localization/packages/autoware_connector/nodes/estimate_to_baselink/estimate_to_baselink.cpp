...
private:
	...
	ros::Subscriber sub_config_, sub_estimate_, sub_odometry_, sub_twist_;
	...
	void callbackOdometry(const nav_msgs::Odometry &msg)
	{
...

		{
			...
			po_rot += Eigen::Vector3d(msg.twist.twist.linear.x * time_diff, 0, 0);
	...
	void callbackVehicleCmd(const autoware_msgs::VehicleCmd &msg)
	{
		...
		Eigen::Quaterniond odom_qua = Eigen::Quaterniond(Eigen::AngleAxisd(msg.ctrl_cmd.steering_angle/10, Eigen::Vector3d::UnitZ()));
