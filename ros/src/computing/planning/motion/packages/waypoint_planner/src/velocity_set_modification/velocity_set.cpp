...
#include <waypoint_planner/velocity_set_modification/libvelocity_set.h>
#include <waypoint_planner/velocity_set_modification/velocity_set_info.h>
#include <waypoint_planner/velocity_set_modification/velocity_set_path.h>
...
constexpr double STOP_SEARCH_DISTANCE = 200;

autoware_msgs::DetectedObjectArray object_tracker;
ros::Publisher obstacle_velocity_pub, use_mobileye_obstacle_pub;
ros::Publisher pillar_velocity_pub, obstacle_type_pub;

const autoware_msgs::MobileyeObstacle mobileye_obstacle;
double mobileye_velocity, mobileye_posx;
int mobileye_obstacle_status;
double pillar_velocity;
double obstacle_velocity;
double vehicle_vel;
int mobileye_process_rute;
...
void obstacleColorByKind(const EControl kind, std_msgs::ColorRGBA &color, const double alpha=0.3)
{
                       ...
                       const bool use_point_cloud, const bool use_point_pillar, const bool use_mobileye)
{
  ...
  *obstacle_type = (int)EObstacleType::NONE;
  ...
  for (int i = closest_waypoint; i < closest_waypoint + STOP_SEARCH_DISTANCE; i++)
  {
    ...
    if(!check_point && !check_pillar && !check_mobileye)
    {
          ...
          lane.waypoints.at(i).gid == wpidx_detection_result_by_other_nodes)
      {
        ...
        *obstacle_type |= (int)EObstacleType::STOPLINE;
      ...
      if (i == crosswalk.getDetectionWaypoint())
      {
        ...
        {
          stop_obstacle_waypoint = i;
          *obstacle_type |= (int)EObstacleType::ON_CROSSWALK;
          break;
        }
      }
    }
    ...
    if(!check_point && use_point_cloud)
    {
      ...
      {
        if(!check_pillar || !check_mobileye) stop_obstacle_waypoint = i;
        *obstacle_type |= (int)EObstacleType::ON_WAYPOINTS;
        check_point = true;
        end_waypoint = i + 1;
	  ...
	  if(!check_pillar && !check_mobileye && use_point_pillar)
    {
          ...
          for(int cou2=0; cou2<mesh; cou2++)
          {
            for(int cou3=0; cou3<mesh; cou3++)
            {
              ...
              if (dt < stop_range)
              {
                ...
                *obstacle_type |= (int)EObstacleType::ON_POINT_PILLAR;
      ...
      std::cout << "min_dt : " << min_dt << "," << *obstacle_type << std::endl;
      if(*obstacle_type & (int)EObstacleType::ON_POINT_PILLAR)
      {
    ...
    {//std::cout << mobileye_obstacle.data.size() << std::endl;
      for(int obj_i=0; obj_i<mobileye_obstacle.data.size(); obj_i++)
      {
        ...
        for(int wid=0; wid<3; wid++)
        {
          ...
          //listener->waitForTransform("map", str.str().c_str(), nowtime, ros::Duration(0.1));
          if(listener->waitForTransform("map", str.str().c_str(), nowtime, ros::Duration(0.1)) == false)
            continue;
          try
          {
          ...
          {
            std::cout << "loolup error : " << ex.what() << std::endl;
            continue;
          }
          
          geometry_msgs::Point pm;
          pm.x = tf_detect.getOrigin().getX();  pm.y = tf_detect.getOrigin().getY();  pm.z = tf_detect.getOrigin().getZ();
          geometry_msgs::Point pml = calcRelativeCoordinate(pm, localizer_pose.pose);
          tf::Vector3 point_vector(pml.x, pml.y, 0);
          double dt = tf::tfDistance(point_vector, tf_waypoint);
//std::cout << stop_range << "," << dt << std::endl;
          if (dt < stop_range)
          {
            *obstacle_type |= (int)EObstacleType::ON_MOBILEYE;
...
EControl pointsDetection(const pcl::PointCloud<pcl::PointXYZ>& points, const int closest_waypoint,
                         ...
                         int* obstacle_waypoint, ObstaclePoints* obstacle_points, int *ob_type)
{
  ...
  int obstacle_type = (int)EObstacleType::NONE;
  ...
  *ob_type = obstacle_type;
  ...
  if (vs_info.getDecelerationRange() < 0.01)
  {
    ...
    *obstacle_waypoint = stop_obstacle_waypoint;
	  ...
	  if(obstacle_type == (int)EObstacleType::ON_CROSSWALK)
		return EControl::STOP;
	  else if (obstacle_type == (int)EObstacleType::STOPLINE)
		return EControl::STOPLINE;
    else if(obstacle_type & (int)EObstacleType::ON_MOBILEYE)
    {
        std_msgs::Float64 vel;

        const double mobileye_velocity_th = 0.0;
        vehicle_vel = vs_info.getCanVelocity();
        obstacle_velocity = std::min(vehicle_vel + mobileye_velocity, 0.0);//前方車両の速度
        if(mobileye_obstacle_status == 1 || mobileye_obstacle_status == 2 || mobileye_obstacle_status == 5)//前方車両が停止中
        {
          mobileye_process_rute = 1;
          obstacle_velocity = 0;
          vel.data = obstacle_velocity;
		      obstacle_velocity_pub.publish(vel);
          return EControl::STOP;
        }
        else if(mobileye_velocity > mobileye_velocity_th)
        {
          //if(vs_path.getCurrentVelocity() > 0.020)
          mobileye_process_rute = 2;
           return EControl::KEEP;
        }
        else
        {
          const double posx_offset = 1;
          if(mobileye_posx > vehicle_vel / 2 + posx_offset)
          {
            mobileye_process_rute = 3;
            return EControl::KEEP;//距離が自車速度の半分の場合はKEEP
          }
          std::cout << vehicle_vel << "," << mobileye_velocity << "," << obstacle_velocity << std::endl;
          if(mobileye_velocity < -5.0 /3.6  && mobileye_posx  >  vehicle_vel / 5.0 + posx_offset)
          {
            mobileye_process_rute = 4;
            vel.data = obstacle_velocity;
		        obstacle_velocity_pub.publish(vel);
            return EControl::DECELERATE_CRUSE;
          }
          else
          {
            mobileye_process_rute = 5;
            obstacle_velocity = 0;
            vel.data = obstacle_velocity;
		        obstacle_velocity_pub.publish(vel);
            return EControl::STOP;
	  ...
	  else if(obstacle_type & (int)EObstacleType::ON_POINT_PILLAR)
    {
		  	std_msgs::Float64 vel;
	  ...
	  else if(obstacle_type & (int)EObstacleType::ON_WAYPOINTS)
	  {
...
void obstacleTypeView(const int obstacle_type, EControl mode, const VelocitySetInfo& vs_info)
{
	...
	switch(obstacle_type)
	{
      ...
      case (int)EObstacleType::ON_MOBILEYE:
	    {
		    std::stringstream str;
			  str << std::fixed << std::setprecision(3) << "MOBILEYE : mobileye velocity " << mobileye_velocity * 3.6 << " current_velocity " << vs_info.getCanVelocity() * 3.6;
        str << std::fixed << std::setprecision(3) << " vplus " << obstacle_velocity;
        str << " mode " << (int)mode << " status " << mobileye_obstacle_status << " mpos_x " << mobileye_posx << " process " << mobileye_process_rute;
			  obstacle_type_text.text = str.str();
			  break;
	    }
	    ...
	    {
		    std::stringstream str;
			  str << "WAYPOINT & MOBILEYE : mobileye velocity " << mobileye_velocity * 3.6 << " mode " << (int)mode << std::endl;
			  obstacle_type_text.text = str.str();
			  break;
	    }
	}
                           ...
                           const ros::Publisher& obstacle_pub, int* obstacle_waypoint)
{
  ...
  EControl detection_result = pointsDetection(vs_info.getPoints(), closest_waypoint, lane, crosswalk, vs_info,
                                              obstacle_waypoint, &obstacle_points, &obstacle_type);
  ...
  obstacleTypeView(obstacle_type, detection_result, vs_info);
...
void changeWaypoints(const VelocitySetInfo& vs_info, const EControl& detection_result, int closest_waypoint,
                     int obstacle_waypoint, const ros::Publisher& final_waypoints_pub, VelocitySetPath* vs_path, ros::Publisher tmp_pub)
{
  ...
  {  // DECELERATE for obstacles
    ...
    else if(detection_result == EControl::DECELERATE_CRUSE)
  {
    vs_path->changeWaypointsForDecelerationCruse(vs_info.getDecelerationObstacle(), closest_waypoint, obstacle_waypoint,
                                                 obstacle_velocity);
    vs_path->avoidSuddenDeceleration(vs_info.getVelocityChangeLimit(), vs_info.getDecelerationObstacle(), closest_waypoint);
    vs_path->avoidSuddenAcceleration(vs_info.getDecelerationObstacle(), closest_waypoint);
    vs_path->setTemporalWaypoints(vs_info.getTemporalWaypointsSize(), closest_waypoint, vs_info.getControlPose());
  ...
  {  // ACCELERATE or KEEP
    vs_path->initializeNewWaypoints();
    //double magn = vs_path->avoidSuddenAccelerationFluctuation(vs_info.getDecelerationObstacle(), closest_waypoint, tmp_pub);
    //vs_path->avoidSuddenDeceleration(vs_info.getVelocityChangeLimit(), vs_info.getDecelerationObstacle(), closest_waypoint, magn);
    vs_path->avoidSuddenAcceleration(vs_info.getDecelerationObstacle(), closest_waypoint);
    vs_path->avoidSuddenDeceleration(vs_info.getVelocityChangeLimit(), vs_info.getDecelerationObstacle(), closest_waypoint);
    vs_path->setTemporalWaypoints(vs_info.getTemporalWaypointsSize(), closest_waypoint, vs_info.getControlPose());
    final_waypoints_pub.publish(vs_path->getTemporalWaypoints());
...
{
  ros::init(argc, argv, "velocity_set_modification");
  ...
  ros::Subscriber config_sub = nh.subscribe("config/velocity_set_modification", 1, &VelocitySetInfo::configCallback, &vs_info);
  ...
  ros::Publisher alive_pub = nh.advertise<std_msgs::String>("/alive", 1);
  ros::Publisher tmp_pub = nh.advertise<std_msgs::String>("/velocity_set/tmp", 1, true);

  ros::Publisher econtrol_pub = nh.advertise<std_msgs::Int8>("econtrol", 1, false);
  obstacle_velocity_pub = nh.advertise<std_msgs::Float64>("obstacle_velocity", 1);
  use_mobileye_obstacle_pub = nh.advertise<mobileye_560_660_msgs::ObstacleData>("use_mobileye_obstacle", 1);
  pillar_velocity_pub = nh.advertise<std_msgs::Float64>("pillar_velocity", 1);
  obstacle_type_pub = nh.advertise<jsk_rviz_plugins::OverlayText>("obstacle_type", 1);
  ...
  while (ros::ok())
  {
    ...
    node_name.data = "velocity_set";
    alive_pub.publish(node_name);
    ...
    mobileye_obstacle = vs_info.getMobileyeObstacle();
    ...
    changeWaypoints(vs_info, detection_result, closest_waypoint,
                    obstacle_waypoint, final_waypoints_pub, &vs_path, tmp_pub);
