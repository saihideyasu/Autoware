...
void VelocitySetInfo::configCallback(const autoware_config_msgs::ConfigVelocitySetModificationConstPtr &config)
{
...
void VelocitySetInfo::currentVelocityCallback(const geometry_msgs::TwistStamped &msg)
{
  can_velocity_ = msg;
}

void VelocitySetInfo::gnssSpeedCallback(const autoware_msgs::GnssSurfaceSpeed &msg)
{
  can_velocity_.twist.linear.x = msg.surface_speed;
}

void VelocitySetInfo::waypointParamCallback(const autoware_msgs::WaypointParam &msg)
{
  if(msg.obstacle_deceleration > 0) deceleration_obstacle_ = msg.obstacle_deceleration;
}