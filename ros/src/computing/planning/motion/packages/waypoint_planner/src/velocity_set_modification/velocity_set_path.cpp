...
double VelocitySetPath::calcChangedVelocity(const double& current_vel, const double& accel, const std::array<int, 2>& range, double magn) const
{
  static double current_velocity = current_vel;
  static double square_vel = current_vel * current_vel * magn;
  if (current_velocity != current_vel)
  {
    current_velocity = current_vel;
    square_vel = current_vel * current_vel;
                                                          ...
                                                          double velocity_plus)
{
  ...
  for (int index = obstacle_waypoint + extra; index >= closest_waypoint; index--)
  {
    ...
    // v = sqrt( (v0)^2 + 2ax )
    std::array<int, 2> range = {index, obstacle_waypoint};
    double changed_vel = calcChangedVelocity(velocity_plus * velocity_plus, deceleration, range);
...
double VelocitySetPath::avoidSuddenAccelerationFluctuation(double deceleration, int closest_waypoint, ros::Publisher tmp_pub)
{
  static int count = 0;
  const double max_vel_magn = 2;
  const double max_vel_fluctuation = 30.0/3.6;
  const double min_vel_fluctuation = 10.0/3.6;
  double magn = 1;
  double vel = new_waypoints_.waypoints[closest_waypoint].twist.twist.linear.x - current_vel_;
  if(vel > min_vel_fluctuation && current_vel_ < 10.0/3.6)
  {
    magn = (max_vel_magn - 1.0) * (vel - min_vel_fluctuation) / (max_vel_fluctuation - min_vel_fluctuation) + 1.0;
    if(magn > max_vel_magn) magn = max_vel_magn;
    std_msgs::String str;
    char buf[100];
    sprintf(buf, ",%d,%lf,x", count, magn);
    str.data = buf;
    tmp_pub.publish(str);
  }
  //std::cout << magn << "," << new_waypoints_.waypoints[closest_waypoint].twist.twist.linear.x << "," << current_vel_ << std::endl;
count++;
  for (int i = 0;; i++)
  {
    ...
    double changed_vel = calcChangedVelocity(current_vel_, deceleration, range) * magn + velocity_offset_;
...
void VelocitySetPath::avoidSuddenDeceleration(double velocity_change_limit, double deceleration, int closest_waypoint, double magn)
{
  if (closest_waypoint < 0)
    return;
  ...
  const double& closest_vel = new_waypoints_.waypoints[closest_waypoint].twist.twist.linear.x;
  // not avoid braking
  std::cout << velocity_change_limit << "," << current_vel_ << "," << closest_vel << std::endl;
  if (std::abs(current_vel_ - closest_vel) < velocity_change_limit * magn)
    return;
