...
class VelocitySetPath
{
 ...
 public:
  ...
  double calcChangedVelocity(const double& current_vel, const double& accel, const std::array<int, 2>& range, double magn) const;
  ...
  void avoidSuddenDeceleration(double velocity_change_limit, double deceleration, int closest_waypoint, double magn);
  void avoidSuddenAcceleration(double decelerationint, int closest_waypoint);
  double avoidSuddenAccelerationFluctuation(double decelerationint, int closest_waypoint, ros::Publisher tmp_pub);
  ...
  void resetFlag();
  void changeWaypointsForDecelerationCruse(double deceleration, int closest_waypoint, int obstacle_waypoint, double velocity_plus);
