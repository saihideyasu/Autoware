...
#include "autoware_config_msgs/ConfigRayGroundFilterBox.h"
...
void RayGroundFilterBox::update_config_params(const autoware_config_msgs::ConfigRayGroundFilterBox::ConstPtr& param)
{
...
void RayGroundFilterBox::publish_cloud(const ros::Publisher& in_publisher,
 ...
 */
void RayGroundFilterBox::ConvertXYZIToRTZColor(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud,
 ...
 */
void RayGroundFilterBox::ClassifyPointCloud(std::vector<PointCloudXYZIRTColor>& in_radial_ordered_clouds,
 ...
 */
void RayGroundFilterBox::ClipCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr,
 ...
 */
void RayGroundFilterBox::ExtractPointsIndices(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr,
 ...
 */
void RayGroundFilterBox::RemovePointsUpTo(const pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr,
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr out_filtered_cloud_ptr)
{
  ...
  tf::Vector3 baselink_point = velodyne_to_baselink_tf.getOrigin();
  std::cout << baselink_point.y() << std::endl;
  ...
  double length_max = vehicle_front_to_baselink_ + baselink_point.x();
  double length_min = vehicle_front_to_baselink_ - vehicle_length_ + baselink_point.x();
  double width_right = vehicle_width_ / 2.0 + baselink_point.y();
  double width_left = -vehicle_width_ / 2.0 + baselink_point.y();
  double height_max = vehicle_height_ + baselink_point.z();
  double height_min = 0 + baselink_point.z();
  std::cout << "length : " << length_max << "," << length_min << std::endl;
  std::cout << "width : " << width_right << "," << width_left << std::endl;
  std::cout << "height: " << height_max << "," << height_min << std::endl;
#pragma omp for
  for (size_t i=0; i< in_cloud_ptr->points.size(); i++)
  {
    ...
    if(x <= length_max && x >= length_min &&
       y <= width_right && y >= width_left &&
       z <= height_max && z >= height_min)
    {
      indices.indices.push_back(i);
    }
  }
...
void RayGroundFilterBox::CloudCallback(const sensor_msgs::PointCloud2ConstPtr &in_sensor_cloud)
{
...
RayGroundFilterBox::RayGroundFilterBox()
  :node_handle_("~")
  ,global_node_handle_()
{
  ros::NodeHandle nh;
...
void RayGroundFilterBox::Run()
{
  ...
  //global params
  global_node_handle_.param("/vehicle_info/vehicle_length", vehicle_length_, 6.5);
  global_node_handle_.param("/vehicle_info/vehicle_width", vehicle_width_, 2.2);
  global_node_handle_.param("/vehicle_info/vehicle_height", vehicle_height_, 2.55);
  global_node_handle_.param("/vehicle_info/front_bumper_to_baselink", vehicle_front_to_baselink_, 4.55);
  ...
  points_node_sub_ = node_handle_.subscribe(input_point_topic_, 1, &RayGroundFilterBox::CloudCallback, this);

  config_node_sub_ = node_handle_.subscribe("/config/ray_ground_filter_box", 1, &RayGroundFilterBox::update_config_params, this);
